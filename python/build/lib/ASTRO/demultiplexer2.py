        ...
        for line in infile:
            if not line.startswith('@'):
                ...
                if read_name not in reads_to_remove:
                    read1seq, read2seq   = arrays[1].split('-', maxsplit=1)
                    lenread1 = len(read1seq)
                    if lenread1 <  10:
                        continue
                    UMIseq = arrays[2]
                    read1qual, read2qual = arrays[3].split('-', maxsplit=1)
...
def merge_chunk_to_tempfilePair(chunk_index, linesA, linesB, linesB2, linesC, lowlength, highlength, temp_dir):
        with tempfile.NamedTemporaryFile(mode='w', delete=False, dir=temp_dir, prefix=f"chunk_{chunk_index}_", suffix=".fastq") as tf:
            ...
            for i in range(0, len(linesA), 4):
                ...
                new_read_name = f"@{chunk_index}_{ii}--{b2}-{b22}--{c2}--{b4}-{b24}"
                ii += 1
                ...
                if lowlength <= len(new_line2) <= highlength:
...
def Fqs2_2fq(fa, fb, fb2, fc, out, nproc, chunk_size, temp_dir, lowlength = 20, highlength = 60):
        ...
        temp_files = []  
        with open(fa, 'r') as fA, open(fb, 'r') as fB, open(fb2, 'r') as fB2, open(fc, 'r') as fC:
            async_results = []
            ...
            for r in async_results:
                file1, file2 = r.get()
                temp_files.extend([file1, file2])

            pool.join()

        with open(out, 'w') as fw, open(out, 'w') as fw2:
            for tf in temp_files:
                with open(tf[0], 'r') as f:
                    for line in f:
                        fw.write(line)
...
def demultiplexingPair(R1, R2, barcode_file, PrimerStructure1, StructureUMI, StructureBarcode, threadnum, outputfolder):
    ...
    Fqs2_2fq(index_fq, Cleanr1Fq1, RemainFq2, UMI_fq,CombineFq1,16,1000000,temps_path)
