...
def demultiplexingPair(
    ...
    with open(barcode_file, "r") as barcodes_in, open(
        barcode_db_fa, "w"
    ) as barcode_db_file:
    ...
    subprocess.run(
        [
            ...
            CombineFq1,
            "--outFileNamePrefix",
            os.path.join(outputfolder, "temps/barcodeMapping/temp"),
            "--outSAMtype",
            "SAM",
    ...
    filter_samPair(
        os.path.join(outputfolder, "temps/barcodeMapping/tempAligned.out.sam"),
        CombineFq1,
        CombineFq2,
        int(threadnum),
        1000000,
    ...
    os.replace(
        os.path.join(outputfolder, "temps/barcodeMapping/tempLog.final.out"),
        os.path.join(outputfolder, "barcodeMapping.out"),
    )
