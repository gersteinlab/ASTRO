...
def dedup_bam_samtools_markdup(input_bam, output_bam, threads=1, log_function=print):

    regex = r'.+\|:_:\|(.+)$'
    temp_sam = f"{output_bam}.temp.sam"
    ...
    log_function(f"[dedup_bam_v1] Step1: Running markdup => {' '.join(markdup_cmd)}")
    ...
    with open(temp_sam, "r") as f_in:
        for line in f_in:
            ...
            if (flag & 1024) != 0:  
                ...
                if splitted:
                    old_readname = splitted[0]
                else:
                    old_readname = qname
                dedup_dict[old_readname] = True

    cmd_samview_output = ["samtools", "view", "-b", "-o", output_bam, "-"]
    log_function(f"[dedup_bam_v1] Step2: Output to {output_bam}")

    with open(temp_sam, "r") as f_in, \
         subprocess.Popen(cmd_samview_output, stdin=subprocess.PIPE, text=True) as proc_out:
        ...
        for line in f_in:
            ...
            splitted = qname.split("|:_:|", 1)
            if len(splitted) == 2:
                oldNamePart, posPart = splitted[0], splitted[1]
            else:
                oldNamePart, posPart = "", ""
            ...
            outline = "\t".join(fields) + "\n"
            proc_out.stdin.write(outline)

        proc_out.stdin.close()
        ret2 = proc_out.wait()
        if ret2 != 0:
            raise RuntimeError("[Error] Writing cleaned BAM via samtools view failed")

    if os.path.exists(temp_sam):
        os.remove(temp_sam)

    log_function(f"[dedup_bam_v1] Done => {output_bam}")
...
def parallel_dedup(cmd_collated_view, cmd_final_write, nproc=4, chunk_size=100000, temp_dir="/tmp"):
    ...
    def flush_buffer_to_pool(buf, idx):
        res = pool.apply_async(dedup_chunk, (idx, buf, temp_dir))
        return res
...
def collate_bam(input_bam, output_bam, threads=1, log_function=print):
    ...
    result = subprocess.run([ "samtools", "collate", "-@", str(threads), "-o", output_bam, input_bam])
...
def genomemapping(starref, gtffile, threadnum, options, outputfolder, STARparamfile='NA'):
    ...
    if "M" in options:
        my_logger("[genomemapping] Using samtools markdup_cmd to remove duplicates")
        dedup_bam_samtools_markdup(
        ...
        my_logger("[genomemapping] Using alignment performance => remove duplicates")
